...
OUTPUT_DIR = Path(__file__).parent.parent / "extractions" / "test_jiyue_only"
...
def main():
    ...
    # 预估
    avg_time = 1.6  # 秒/对话（根据JY测试）
    total_time_seconds = len(target_sessions) * avg_time / PARALLEL_WORKERS
    total_time_minutes = total_time_seconds / 60
    estimated_cost = len(target_sessions) * 0.00065  # 根据JY测试实际成本
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            if completed % 100 == 0:
                ...
                eta = (len(target_sessions) - completed) / rate if rate > 0 else 0

                print(f"\n📊 进度: {completed}/{len(target_sessions)} ({completed/len(target_sessions)*100:.1f}%)")
    ...
    print(f"  - 平均速度: {len(target_sessions)/total_time:.2f} 对话/秒")
    print(f"  - 平均耗时: {total_time/len(target_sessions):.2f} 秒/对话")
...
def process_single(session, index, total):
    ...
    # 只在失败时打印
    if status == 'failed':
        print(f"  ❌ [{index}/{total}] 失败: {conv_name}")
