...
OUTPUT_DIR = Path("extractions/test_three_users")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 测试目标用户
TARGET_USERS = ["吉月", "JY", "weiwei"]
...
def main():
    ...
    # 过滤目标用户的对话
    target_sessions = []
    user_counts = {user: 0 for user in TARGET_USERS}
    ...
    for item in metadata:
        ...
        # 只提取目标用户
        if conv_name in TARGET_USERS:
            target_sessions.append(item)
            user_counts[conv_name] += 1
    ...
    for user in TARGET_USERS:
        ...
        print(f"  - {user}: {count:,} 对话")

    print("=" * 60)
    print(f"  总计: {total_conversations:,} 对话")
    print()
    ...
    # 预估
    avg_time = 28  # 秒/对话
    total_time_seconds = total_conversations * avg_time / PARALLEL_WORKERS
    total_time_minutes = total_time_seconds / 60
    estimated_cost = total_conversations * 0.001434

    print("⏱️ 预估:")
    print(f"  - 耗时: {total_time_minutes:.1f} 分钟 ({total_time_minutes/60:.2f} 小时)")
    print(f"  - 成本: ${estimated_cost:.2f}")
    print()

    # 确认
    response = input("开始提取？(yes/no): ")
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        ...
        for idx, session in enumerate(target_sessions):
            future = executor.submit(process_single, session, idx + 1, total_conversations)
        ...
        for future in as_completed(futures):
            ...
            if completed % 50 == 0:
                ...
                eta = (total_conversations - completed) / rate if rate > 0 else 0

                print(f"\n📊 进度: {completed}/{total_conversations} ({completed/total_conversations*100:.1f}%)")
                print(f"   成功: {stats['success']}, 失败: {stats['failed']}")
                print(f"   速度: {rate:.1f} 对话/秒, 预计剩余: {eta/60:.1f} 分钟")
    ...
    print(f"  - 平均速度: {total_conversations/total_time:.1f} 对话/秒")
