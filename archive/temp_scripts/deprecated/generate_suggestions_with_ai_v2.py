...
print("=" * 80)
print("使用Vertex AI生成Person合并建议 (改进版)")
print("=" * 80)
...
# 改进的prompt
MERGE_PROMPT = """分析以下微信对话中的Person实体，判断哪些应该合并。
...
4. 纯关系词不合并（如"妈妈"在不同上下文可能指不同的人）

请返回JSON格式（不要用markdown代码块包裹，直接返回JSON）：
{{
  "merge_groups": [
    {{
      "suggested_name": "建议使用的名字",
      "reason": "合并原因（一句话，不要换行）",
      "variants": ["人名1", "人名2"]
    }}
  ]
}}
...
def analyze_conversation_with_ai(conv_name, person_names_set):
    """使用AI分析一个对话的合并建议"""
    global success_count, failed_count, failed_conversations
    ...
    try:
        ...
        if result is None:
            failed_count += 1
            failed_conversations.append(conv_name)
            return None
        ...
        failed_count += 1
        failed_conversations.append(f"{conv_name} (错误: {str(e)[:50]})")
        return None
...
with ThreadPoolExecutor(max_workers=10) as executor:
    ...
    for future in as_completed(futures):
        ...
        if completed % 50 == 0 or completed == total:
            print(f"  进度: {completed}/{total} (成功: {success_count}, 失败: {failed_count})")
...
if failed_conversations:
    print(f"\n失败的对话（前10个）:")
    for conv in failed_conversations[:10]:
        print(f"  - {conv}")
