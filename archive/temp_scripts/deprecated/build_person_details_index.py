#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""为每个Person实体构建详细信息索引（关联的events和relationships）"""
import json
import pickle
import sys
...
# 2. 构建索引结构
safe_print("\n2. 初始化索引结构...")
person_details = {}  # person_id -> {events: [], relationships: []}
...
for i, file_path in enumerate(files):
    ...
    try:
        ...
        for event in entities.get('events', []):
            ...
            # 找到这些participant对应的person_ids
            for participant_name in participants:
                for pid, person in enumerate(persons):
                    if (person['name'] == participant_name and
                        person['conversation'] == conv_name and
                        person['source_file'] == file_path.name):

                        person_details[pid]['events'].append({
                            'name': event.get('name', ''),
                            'type': event.get('type', ''),
        ...
        for rel in entities.get('relationships', []):
            ...
            # 找source和target对应的person_ids
            for pid, person in enumerate(persons):
                if person['conversation'] != conv_name:
                    continue
                if person['source_file'] != file_path.name:
                    continue

                # 如果这个person是source或target
                ...
                elif person['name'] == target_name and rel.get('target_type') == 'Person':
                    person_details[pid]['relationships'].append({
                        'type': rel_type,
                        'role': 'target',
                        'other': source_name,
        ...
        # 3.3 处理topics (通过DISCUSSED_TOPIC关系)
        for rel in entities.get('relationships', []):
            if rel.get('type') == 'DISCUSSED_TOPIC':
                source_name = rel.get('source', '')
                topic_name = rel.get('target', '')

                for pid, person in enumerate(persons):
                    if (person['name'] == source_name and
                        person['conversation'] == conv_name and
                        person['source_file'] == file_path.name):
...
safe_print(f"   [OK] 成功处理 {processed} 个文件")
