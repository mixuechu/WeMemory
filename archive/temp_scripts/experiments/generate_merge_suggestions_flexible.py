...
if USE_VERTEX_AI:
    ...
    VERTEX_PROJECT = os.getenv('VERTEX_PROJECT')  # 从环境变量读取

    if not VERTEX_PROJECT:
        print("❌ 错误: 请设置环境变量 VERTEX_PROJECT")
        print("   export VERTEX_PROJECT=your-project-id")
        exit(1)

    print(f"   Project: {VERTEX_PROJECT}")
    print(f"   Region: {VERTEX_REGION}")
    client = AnthropicVertex(region=VERTEX_REGION, project_id=VERTEX_PROJECT)
...
final_output = {
    'metadata': {
        ...
        'model': MODEL,
        'api_type': 'vertex_ai' if USE_VERTEX_AI else 'anthropic'
    },
