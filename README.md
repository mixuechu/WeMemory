...
> **项目状态 (2026-03-09)**: 已从全量数据（676对话）迁移到精简版（138对话），采用 text-multilingual-embedding-002 模型和三元组知识图谱。全量数据已备份至 `/archive/full_data_backup/`。
...
- ✅ **三元组知识图谱**：7,865 条优化三元组（5,297 事件 + 2,568 关系）
- ✅ **多语言 Embedding**：text-multilingual-embedding-002 模型，显著提升中文区分度
- ✅ **混合检索**：BM25（50%）+ 向量检索（50%），平衡关键词匹配和语义理解
- ✅ **智能切分**：混合滑动窗口策略，生成主 sessions + 边界 overlaps
- ✅ **FAISS 加速**：HNSW 索引实现 100-400x 搜索加速
- ✅ **精简数据集**：138 个精选对话，易于版本控制和快速迭代
...
│   │   ├── curated_kg.json       # 精选知识图谱
│   │   ├── triplets.json         # 三元组数据（7,865条）
│   │   └── entity_alias_map.json # 实体别名映射
│   └── conversations/        # 对话数据（138个）
│
├── vector_stores/            # 向量库（生产）
│   ├── triplets/             # 三元组向量索引
│   │   ├── embeddings.pkl
│   │   └── index.faiss
...
│
├── tests/                    # 测试代码
│   ├── test_triplet_search.py    # 三元组搜索测试
│   ├── comprehensive_test.py     # 综合测试（111个查询）
│   └── README.md
│
├── archive/                  # 归档目录（不上传Git）
...
├── scripts/                  # 脚本工具
├── data_loader/              # 数据加载模块
├── embedding/                # 向量生成模块（已升级 multilingual）
├── retrieval/                # 检索模块
├── api/                      # API 服务
├── examples/                 # 使用示例
├── docs/                     # 技术文档
...
生成多语言向量 embeddings，优化中文语义理解。

**特性**：
- Google Vertex AI **text-multilingual-embedding-002**（768 维）
- **显著提升中文区分度**（相比 text-embedding-004）
