...
personal_friends = [item for item in data if not is_group(item['name'])]

print('Top 20 个人好友对话数量：')
print('=' * 70)
print(f"{'排名':>4} | {'名称':40} | {'对话数':>8}")
print('-' * 70)

for i, item in enumerate(personal_friends[:20], 1):
    rank = item['rank']  # 原始排名
    name = item['name']
...
# 统计
total_personal_top20 = sum(item['conversations'] for item in personal_friends[:20])
print(f"\n统计：")
print(f"  - Top20个人好友总对话数: {total_personal_top20:,}")
print(f"  - 占全部对话比例: {total_personal_top20/183287*100:.1f}%")
print(f"  - 总个人好友数: {len(personal_friends)}")
print(f"  - 总群聊数: {len(data) - len(personal_friends)}")
